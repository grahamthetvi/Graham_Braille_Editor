...
- **ChromeOS / WebUSB:** Print directly from the browser — no Bridge. The ViewPlus Max is a USB printer, so ChromeOS will claim it after the first `close()`. Graham keeps the USB port open after a successful grab. If you see **Access Denied**, leave this tab open, unplug the Max, remove it under **Settings → Print → Printers** while unplugged, then plug it back in. Print bar **Debug** (or `?usbDebug=1` / Ctrl+Shift+Alt+U) shows whether the port is held. After it works, do not unplug and do not close the tab.
