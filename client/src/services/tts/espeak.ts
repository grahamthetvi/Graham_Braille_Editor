...
export async function synthesizeEspeakPcm(
  ...
  onProgress?.({ phase: 'loading', messageKey: 'app.file.downloadMp3.status.loadingEspeak' });
  const factory = await loadESpeakFactory();

  onProgress?.({ phase: 'synthesizing', messageKey: 'app.file.downloadMp3.status.synthesizingEspeak' });
